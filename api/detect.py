...
def detect(source, save_dir, user_id=123456, aws_s3_path="./media/aws_s3_bucket/", save_img=False):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    print("save dir + save_dir + image_name", save_dir + image_name)
                    cv2.imwrite(save_dir + image_name, im0) # media
    ...
    print("Detections: ", detectionsLabel)
    print("Type of lable: ", type(detectionsLabel))
    if save_txt or save_img:
        s = f"\n{len(list(save_dir.glob('labels/*.txt')))} labels saved to {save_dir / 'labels'}" if save_txt else ''
        print(f"Results saved to {save_dir}{s}")

    print(f'Done. ({time.time() - t0:.3f}s)')
    return {"new_path": new_path, "Detections": detectionsLabel}


# if __name__ == '__main__':
#     source = "test_vinbigdata.png"
#     save_dir = "Output"
#     user_id = "123456"
#     aws_s3_path = "Output\\"

#     detect(source, save_dir, user_id, aws_s3_path)

    # !python detect.py --weights '/content/yolov5/runs/train/exp/weights/best.pt' --img 640 --conf 0.15 --iou 0.5 --source /content/C.png --exist-ok
